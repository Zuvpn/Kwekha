...
choose_scheme_menu() {
  # This function is used like: scheme="$(choose_scheme_menu)"
  # So: print UI to STDERR (still visible), and only echo final scheme to STDOUT.
  {
    echo
...
EOF
  ...
  case "$c" in
    ...
    24) echo "udp" ;;
    25)
      read -rp "scheme دلخواه را بنویس (مثلاً relay+wss): " ss >&2
      echo "${ss:-relay+wss}"
      ;;
    *) echo "relay+wss" ;;
  esac
...
quick_setup_wizard() {
  ...
  local scheme; scheme="$(choose_scheme_menu)"
  [[ -n "$scheme" ]] || scheme="relay+wss"
