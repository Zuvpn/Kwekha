...
check_loop_warning() {
  ...
  for p in "${_ps[@]}"; do
    ...
    if ss -lntp 2>/dev/null | grep -qE "[:.]${p}\s" && ss -lntp 2>/dev/null | grep -E "[:.]${p}\s" | grep -qi "gost"; then
      echo
      echo "⚠️ هشدار: به نظر می‌رسد پورت ${p} روی localhost توسط خود gost اشغال است."
      echo "   اگر پشت این پورت سرویس واقعی (xray/nginx/panel) ندارید، خطای Host Unreachable خواهید دید."
      echo "   برای سناریو «پنل روی خارج»، مقصد را گزینه 2 (سرور مقابل) انتخاب کنید."
      echo
      break
    fi
  done
...
choose_dest_mode() {
  # 1 = localhost (default), 2 = remote host
  echo
  echo "📍 مقصد فوروارد کجاست؟"
  echo "  1) روی همین سرور (127.0.0.1)  [پیش‌فرض]"
  echo "  2) روی سرور مقابل (IP/Domain)"
...
choose_dest_host() {
  ...
  if [[ "$mode" == "remote" ]]; then
    read -rp "🌐 IP/Domain مقصد (پیش‌فرض: ${default_remote}): " dh
...
echo
  ...
  # Destination (where forwarded ports should land)
  local dest_mode dest_host
  dest_mode="$(choose_dest_mode)"
  local default_remote="${kharej_host:-}"
  dest_host="$(choose_dest_host "$dest_mode" "$default_remote")"
