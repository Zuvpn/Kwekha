...
choose_dest_mode() {
  # 1 = local (iran/localhost), 2 = remote (foreign/remote host)
  echo
  echo "🧩 سرویس مقصد (مثل Xray/Nginx/Panel) کجاست؟"
  echo "  1) روی همین سرور (ایران - 127.0.0.1)"
  echo "     ➜ اگر Xray روی همین سرور اجرا می‌شود"
  echo ""
  # اگر REMOTE_HOST ست نشده بود، فقط عبارت Remote را نشان بده
  if [[ -n "${REMOTE_HOST:-}" ]]; then
    echo "  2) روی سرور خارج (${REMOTE_HOST})"
  else
    echo "  2) روی سرور خارج (Remote server)"
  fi
  echo "     ➜ اگر Xray روی سرور خارج اجرا می‌شود"
  echo ""
