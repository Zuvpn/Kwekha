...
KWEKHA_CMD_PATH="/usr/local/bin/kwekha"
...
EOF
  ...
  case "$c" in
    1) CHOSEN_SCHEME="relay+wss" ;;
    2) CHOSEN_SCHEME="relay+tls" ;;
    3) CHOSEN_SCHEME="grpc" ;;
    4) CHOSEN_SCHEME="h2" ;;
    5) CHOSEN_SCHEME="tcp" ;;
    6)
      echo
      echo "Advanced protocols:"
...
echo
  ...
  if [[ "$role" == "1" ]]; then
    ...
    while IFS= read -r L; do
      ...
      cmd+="-L rtcp://:${p}/:0 "
    done < <(parse_ports_simple "$ports_simple")
...
install_kwekha_cmd() {
  ...
  local url
  url="$(get_update_url)"
  echo "🧩 نصب دستور 'kwekha' در سیستم"
  echo "این کار اسکریپت را در مسیر زیر نصب/آپدیت می‌کند:"
  echo "  $KWEKHA_CMD_PATH"
  echo "URL: $url"
  echo
  local tmp="/tmp/kwekha.$$"
  curl -fsSL "$url" -o "$tmp" || { echo "❌ دانلود ناموفق"; return 1; }
  head -n 5 "$tmp" | grep -qi "K W E K H A" || { echo "❌ فایل دانلودی معتبر نیست."; rm -f "$tmp"; return 1; }
  install -m 755 "$tmp" "$KWEKHA_CMD_PATH"
  rm -f "$tmp"
  echo
  echo "✅ نصب شد. از این به بعد فقط بزن:"
  echo "  sudo kwekha"
}
...
self_update() {
  ...
  rm -f "$tmp"
  echo "✅ آپدیت انجام شد."
  # also update installed command if exists and different path
  if [[ -x "$KWEKHA_CMD_PATH" && "$self" != "$KWEKHA_CMD_PATH" ]]; then
    install -m 755 "$self" "$KWEKHA_CMD_PATH" || true
    echo "ℹ️ همچنین دستور kwekha هم آپدیت شد: $KWEKHA_CMD_PATH"
  fi
...
menu() {
  ...
  echo "17) Install/Update kwekha command"
  echo "18) Uninstall Kwekha (and optional gost)"
  echo "0) Exit"
  echo
  read -rp "Select: " c
  case "$c" in
