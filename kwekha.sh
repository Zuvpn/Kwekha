...
choose_dest_mode() {
...

  {
    ...
    echo "  1) روی همین سرور (ایران - 127.0.0.1)"
    echo "     ➜ اگر Xray/Panel روی همین سرور اجرا می‌شود"
    echo
    if [[ -n "${REMOTE_HOST:-}" ]]; then
      echo "  2) روی سرور خارج (${REMOTE_HOST})"
    else
      echo "  2) روی سرور خارج (Remote server)"
    fi
    echo "     ➜ اگر Xray/Panel روی سرور خارج اجرا می‌شود"
    echo
    printf "انتخاب شما [1 یا 2]: "
  } >"$TTY"
  ...
  case "${dm:-}" in
    ...
    1|"") echo "local" ;;
    *)
      echo "⚠️ ورودی نامعتبر بود؛ پیش‌فرض = 1 (local)." >"$TTY"
      echo "local"
      ;;
  esac
}
...
menu() {
  ...
  echo "7) Scheduler: Configure interval (1m..24h)"
  echo "8) Scheduler: Enable"
...
# =========================
SCHED_CONF="/etc/kwekha/scheduler.conf"
SCHED_SVC="kwekha-scheduler.service"
SCHED_TMR="kwekha-scheduler.timer"

scheduler_write_conf() {
  local interval="${1:-1h}"
...
scheduler_install_units() {
  mkdir -p /etc/systemd/system /etc/kwekha /var/log/kwekha
  cat > /etc/systemd/system/${SCHED_SVC} <<'EOF'
[Unit]
...
EOF
  ...
  cat > /etc/systemd/system/${SCHED_TMR} <<'EOF'
[Unit]
...
scheduler_configure_menu() {
  echo
  echo "⏱️ Scheduler (Force Restart)"
  echo "هر بار اجرا → همه سرویس‌های gost-kwekha-* ریست می‌شوند."
  echo "بازه: 1 دقیقه تا 24 ساعت"
  echo
  echo "  1) 1m     2) 5m     3) 10m     4) 30m"
  echo "  5) 1h     6) 6h     7) 12h     8) 24h"
  read -rp "انتخاب [1-8]: " c
  case "$c" in
    1) scheduler_write_conf "1min" ;;
    2) scheduler_write_conf "5min" ;;
    3) scheduler_write_conf "10min" ;;
    4) scheduler_write_conf "30min" ;;
    5) scheduler_write_conf "1h" ;;
    6) scheduler_write_conf "6h" ;;
    7) scheduler_write_conf "12h" ;;
    8) scheduler_write_conf "24h" ;;
    *) echo "⚠️ انتخاب نامعتبر؛ پیش‌فرض 1h"; scheduler_write_conf "1h" ;;
  esac
  scheduler_install_units
  scheduler_apply_interval
  echo "✅ تنظیم شد. (برای فعال‌سازی: Scheduler Enable)"
}

scheduler_enable() {
  scheduler_install_units
  [[ -f "$SCHED_CONF" ]] || scheduler_write_conf "1h"
  scheduler_apply_interval
  systemctl enable --now "$SCHED_TMR"
  echo "✅ Scheduler فعال شد."
}

scheduler_disable() {
  systemctl disable --now "$SCHED_TMR" 2>/dev/null || true
  echo "✅ Scheduler غیرفعال شد."
}

scheduler_status() {
  echo
  systemctl status "$SCHED_TMR" --no-pager || true
  echo
  systemctl list-timers --all | grep -E "kwekha-scheduler" || true
  echo
  [[ -f "$SCHED_CONF" ]] && { echo "Config:"; cat "$SCHED_CONF"; } || echo "Config: (none)"
}

scheduler_run_now() {
  echo "▶️ اجرای دستی Scheduler..."
  /usr/local/bin/kwekha-scheduler || true
}
...
# =========================
# Telegram Bot (button-only)
# =========================
TG_CONF="/etc/kwekha/telegram.conf"
TG_SVC="kwekha-telegram.service"

telegram_install_unit() {
  mkdir -p /etc/systemd/system /etc/kwekha /var/log/kwekha
  cat > /etc/systemd/system/${TG_SVC} <<'EOF'
[Unit]
Description=Kwekha Telegram Bot
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStart=/usr/local/bin/kwekha-telegram
Restart=always
RestartSec=2
StandardOutput=append:/var/log/kwekha/telegram.log
StandardError=append:/var/log/kwekha/telegram.log
...
telegram_setup_menu() {
  ...
  read -rp "Admin IDs (comma-separated, up to 100): " admins
  read -rp "Allowed IDs (comma-separated, up to 100): " allowed
...
ADMIN_IDS="$admins"
ALLOWED_IDS="$allowed"
DENY_MESSAGE="شما مجاز نیستید"
EOF
  echo "✅ ذخیره شد: $TG_CONF"
  telegram_install_unit
  echo "برای اجرا: Telegram Start bot"
}

telegram_start() {
  telegram_install_unit
  systemctl enable --now "$TG_SVC"
  echo "✅ Telegram Bot started."
}

telegram_stop() {
  systemctl disable --now "$TG_SVC" 2>/dev/null || true
  echo "✅ Telegram Bot stopped."
}

telegram_status() {
  systemctl status "$TG_SVC" --no-pager || true
}
